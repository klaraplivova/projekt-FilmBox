...
premieraFilmu.innerHTML = `Premiéra <strong>${datumPremiery.format(
  "D. M. YYYY"
)}</strong>`;
...
video.addEventListener("timeupdate", () => {
  ...
  let casMinuty = Math.floor(casVidea / 60)
    .toString()
    .padStart(2, "0");
  let casSekundy = Math.floor(casVidea % 60)
    .toString()
    .padStart(2, "0");
  document.querySelector(
    ".current-time"
  ).textContent = `${casMinuty}:${casSekundy}`;
});
